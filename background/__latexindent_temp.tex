...
$\text{[FETCH]}$ will fetch a value in a $\text{ref}_\rho$ by the use of the bang operator, $!L$, and give an object of a shape, $Z$, leading to the effect $\text{read}_\rho$ being added to the set of effects, $\varphi$. Applying $\text{[FETCH]}$ will result in an l-value, which under the environment has shape $\text{ref}_\rho Z$ with the side effects $\varphi$.  

$\text{[ASSIGN]}$ will evaluate both expressions $E_1$ and $E_2$, resulting in effects for both $E_1$, $E_2$ as well as existing effects along with the $\text{write}_\rho$ effect being combined into the effects set. The expression $E_1$ will be written to the location $L$. The left and right-hand sides of the expression, $L$ and $E_1$ must be of the same shape, $Z$, while the result of evaluating $E_2$ can be of a different shape, $Z'$.
