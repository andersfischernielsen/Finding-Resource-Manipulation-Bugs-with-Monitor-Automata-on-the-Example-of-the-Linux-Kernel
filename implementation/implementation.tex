...
\begin{enumerate}
    \item All possible orderings lead to all states of monitors being in an error state 
    \item All possible orderings lead to all states of monitors \underline{not} being in an error state 
    \item All possible orderings lead to \underline{some} states of monitors being in an error state
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{implementation/figures/permutation-copy}
    \caption{An illustration of the exploration of all effect orderings and how copies of an instantiated double-unlock monitor change state given these effect orderings, leading to a possible bug being discovered in one effect ordering, but not the other.}
